...
const igStalk = async (req, res) => {
    try {
        ...
        if (!username) return res.status(400).json({error: "please provide a valid parameter!"});
        let { data } = await axios.get('https://www.instagram.com/'+ username +'/?__a=1',{
            method: 'GET',
            headers: {
...
const igFoto = async (req, res) => {
    ...
    if (!url || !url.includes("http")) return res.status(400).json({error: "please provide a valid parameter!"});
    const foto = await igdl(url, "photo");
    res.json(foto);
}
...
const igVideo = async (req, res) => {
    let url = req.query.url;
    if (!url || !url.includes("http")) return res.status(400).json({error: "please provide a valid parameter!"});
    const foto = await igdl(url, "video");
    res.json(foto);
}

module.exports = { igStalk, igFoto, igVideo }